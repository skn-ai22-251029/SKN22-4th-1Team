...
# Adjust imports for Django project structure
...
class AIService:
        ...
        try:
            res = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": INTENT_CLASS_PROMPT.format(user_query=query),
                    },
                ],
                temperature=0,
    ...
    async def generate_symptom_answer(cls, symptom, data, user_profile=None):
        ...
        if user_profile:
            ...
            logger.debug(
                f"User Profile — Meds: {meds}, Allergies: {allergies}, Diseases: {diseases}"
            )
        ...
        try:
            ...
            return {
                "summary": f"답변 생성 중 오류가 발생했습니다: {str(e)}",
                "ingredients": [],
            }
        ...
        try:
            res = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a medical semantics analyzer.",
                    },
                    {"role": "user", "content": prompt},
                ],
                response_format={"type": "json_object"},
            ...
            return re.sub(
                r"\s+", "_", re.sub(r"[^\w\s가-힣]", "", query.strip())
            ).lower()
        ...
        try:
            res = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a pharmaceutical terminologist.",
                    },
                    {"role": "user", "content": prompt},
                ],
                response_format={"type": "json_object"},
